...
import com.will.shop.bean.order.SubmitOrderOrder;
import com.will.shop.common.constants.Constant;
...
public class SubmitOrderListener {
    ...
    public void defaultSubmitOrderListener(SubmitOrderEvent event) {
        ...
        if (userAddrOrder == null) {
            throw new WillShopBindException("请填写收货地址");
        }
        ...
        skuStocksMap.forEach((key, sku) -> {
            ...
            if (skuMapper.updateStocks(sku) == 0) {
                ...
                throw new WillShopBindException("商品：[" + sku.getProdName() + "]库存不足");
            }
        });
        ...
        prodStocksMap.forEach((prodId, prod) -> {
            ...
            if (productMapper.updateStocks(prod) == 0) {
                ...
                throw new WillShopBindException("商品：[" + prod.getProdName() + "]库存不足");
            }
        });
    ...
    private void createOrder(SubmitOrderEvent event, Date now, String userId, List<Long> basketIds, Map<Long, Sku> skuStocksMap, Map<Long, Product> prodStocksMap, Long addrOrderId, ShopCartOrderDto shopCartOrderDto) {
        ...
        // 订单信息
        com.will.shop.bean.model.Order order = getOrder(now, userId, addrOrderId, shopCartOrderDto, orderNumber, shopId, orderProdName, orderItems);
    ...
    private com.will.shop.bean.model.Order getOrder(Date now, String userId, Long addrOrderId, ShopCartOrderDto shopCartOrderDto, String orderNumber, Long shopId, StringBuilder orderProdName, List<OrderItem> orderItems) {
        com.will.shop.bean.model.Order order = new com.will.shop.bean.model.Order();
        ...
        order.setReduceAmount(Arithmetic.sub(Arithmetic.add(shopCartOrderDto.getTotal(), shopCartOrderDto.getTransfee()), shopCartOrderDto.getActualTotal()));
    ...
    private Product checkAndGetProd(Long prodId, ShopCartItemDto shopCartItem, Map<Long, Product> prodStocksMap) {
        ...
        if (product.getStatus() != 1) {
            throw new WillShopBindException("商品[" + product.getProdName() + "]已下架");
        }
        ...
        if (product.getTotalStocks() != -1 && mapProduct.getTotalStocks() > product.getTotalStocks()) {
            throw new WillShopBindException("商品：[" + product.getProdName() + "]库存不足");
        }
