...
public class CategoryController {
     ...
     */
    @GetMapping("/table")
    @PreAuthorize("@pm.hasPermission('prod:category:page')")
    ...
    @SysLog("保存分类")
    @PostMapping
    @PreAuthorize("@pm.hasPermission('prod:category:save')")
    public ServerResponseEntity<Void> save(@RequestBody Category category) {
        ...
        if (categoryName != null) {
            throw new WillShopBindException("类目名称已存在！");
        }
    ...
    @SysLog("更新分类")
    @PutMapping
    @PreAuthorize("@pm.hasPermission('prod:category:update')")
    public ServerResponseEntity<String> update(@RequestBody Category category) {
        ...
        if (Objects.equals(categoryDb.getStatus(), 0) && Objects.equals(category.getStatus(), 1) && !Objects.equals(category.getParentId(), 0L)) {
            ...
            if (Objects.isNull(parentCategory) || Objects.equals(parentCategory.getStatus(), 0)) {
                ...
                throw new WillShopBindException("修改失败，上级分类不存在或者不为正常状态");
            }
        }
    ...
    @PreAuthorize("@pm.hasPermission('prod:category:delete')")
